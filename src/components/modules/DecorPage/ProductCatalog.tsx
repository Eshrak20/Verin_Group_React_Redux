...
import { INITIAL_VISIBLE, useProductCatalog } from "@/hooks/useProductCatalog";
...
export default function ProductCatalog() {
  ...
  if (!inView || isLoading) {
    return <CatalogSkeleton containerRef={containerRef} />;
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-nowrap sm:flex-wrap overflow-x-auto mb-4 sm:mb-8 gap-2 no-scrollbar">
        {visibleCategories.map((cat) => (
          <button
            ...
            className={`whitespace-nowrap px-5 sm:px-6 py-2 sm:py-2.5 rounded-full font-medium transition-all duration-200 text-xs sm:text-sm
              ${
        ...
        {allSubCategories.length > INITIAL_VISIBLE && (
          <button
            ...
            className="whitespace-nowrap px-5 sm:px-6 py-2 sm:py-2.5 rounded-full font-medium
              transition-all duration-200 text-xs sm:text-sm bg-gray-100 text-gray-600 border border-gray-200 hover:bg-gray-200 hover:text-gray-900
            "
          >
      ...
      {/* Product Grid */}
      <div
        className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 transition-opacity duration-250
...
//   // 🎯 skip: !inView দিয়ে API Call ডিলে করা হয়েছে
//   const { data, isLoading } = useGetProductsQuery(
//     { per_page: 100 },
//     { skip: !inView }
//   );
...
//   // 🎯 Stable Shuffle Algorithm (products পরিবর্তন না হলে র‍্যান্ডম অর্ডার বদলাবে না)
