// src/components/modules/HomePage/DecorFeaturedProducts.tsx
...
export default function DecorFeaturedProducts() {
  const queryArgs = useMemo(() => ({ is_featured: 1 as const, category_id: 1 }), []);
  ...
  const {
    ...
    startAuto,
    VISIBLE,
  } = useProductScroll(products.length);

  if (isLoading) return <FeaturedSkeleton visibleCount={VISIBLE} />;
  ...
  return (
    <section className="py-6 sm:py-8">
      ...
      <div className="relative">
        {canScrollLeft && (
          <button
            ...
            className="absolute left-1 sm:left-2 lg:left-4 top-[40%] -translate-y-1/2 z-10 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/90 dark:bg-slate-700/90 border border-gray-200 dark:border-gray-600 flex items-center justify-center shadow-md hover:cursor-pointer text-gray-700 dark:text-white hover:shadow-lg transition-all"
          >
            <ChevronLeft size={16} />
          </button>
        )}
        ...
        >
          {products.length === 0 ? (
            <div className="w-full text-center py-10 text-gray-400">No featured decor products found.</div>
          ) : (
        ...
        {canScrollRight && (
          <button
            ...
            className="absolute right-1 sm:right-2 lg:right-4 top-[40%] -translate-y-1/2 z-10 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/90 dark:bg-slate-700/90 border border-gray-200 dark:border-gray-600 flex items-center justify-center shadow-md hover:cursor-pointer text-gray-700 dark:text-white hover:shadow-lg transition-all"
          >
      ...
      >
        <Link
          to="/decor"
          className="flex items-center gap-2 bg-[#262626] hover:bg-[#003557] text-white text-xs sm:text-sm font-semibold px-5 sm:px-6 py-2 sm:py-2.5 rounded-full transition-colors duration-200"
        >
          View All Products
          <ChevronRight size={16} />
        </Link>
...
//       if (autoRef.current) clearInterval(autoRef.current);
//     };
//   }, [products.length, checkScroll, startAuto]);
...
//         if (!currentEl) return;
//         const atEnd = currentEl.scrollLeft + currentEl.clientWidth >= currentEl.scrollWidth - 1;
//         if (atEnd) {
//           currentEl.scrollTo({ left: 0, behavior: "smooth" });
//         } else {
//           const cardWidth = getCardWidth();
//           currentEl.scrollBy({ left: cardWidth + GAP, behavior: "smooth" });
//         }
//         setTimeout(checkScroll, 350);
//       }, 7000);
//     }
...
//   const startAuto = () => {
//     if (autoRef.current) clearInterval(autoRef.current);
//     autoRef.current = setInterval(autoScroll, 7000);
//   };
...
//               onMouseEnter={stopAuto}
//               onMouseLeave={startAuto}
//               onTouchStart={stopAuto}  /* 🎯 মোবাইলে স্পর্শ/প্রেস করলেই অটো-স্লাইড স্টপ হবে */
//               onTouchEnd={startAuto}    /* 🎯 স্পর্শ ছেড়ে দিলে বা স্লাইড করা শেষ হলে আবার চালু হবে */
//               className="flex gap-3 overflow-x-auto pb-2"
//               style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
//             >
//               {products.length === 0 ? (
