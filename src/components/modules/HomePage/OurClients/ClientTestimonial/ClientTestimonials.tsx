// src/components/modules/Home/ClientTestimonials.tsx
...
import { type ApiReview } from "@/utils/client.utils";
...
export default function ClientTestimonials() {
  ...
  const { data, isLoading, isError } = useGetReviewsQuery({}, { skip: !inView });
    ...
    <section ref={containerRef} className="py-8 sm:py-10 lg:py-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {latestReviews.map((t, index) => (
          <ClientTestimonialCard key={t.id} review={t} index={index} />
        ))}
      </div>
...
//     .filter((item) => Number(item.is_active) === 1)
//     .sort((a, b) => {
//       // sort_order অনুযায়ী সঠিকভাবে সাজানোর জন্য
//       if (a.sort_order !== b.sort_order) {
//         return a.sort_order - b.sort_order;
//       }
//       return new Date(b.created_at).getTime() - new Date(a.created_at).getTime();
