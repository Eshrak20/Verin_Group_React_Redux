...
import { fakeUsers } from "@/data/user/fakeUser";
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    const existing = getCookie("auth_user");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // email দিয়ে single user খোঁজো (password validation optional এখন)
      const matched = fakeUsers.find((u) => u.email === email);

      if (!matched) {
        setError("এই email দিয়ে কোনো account নেই।");
        setIsLoading(false);
        return;
      }

      // single user object cookie তে save করো — array না
      setCookie("auth_user", JSON.stringify(matched), 7);
      setIsLoading(false);
      navigate("/");
    }, 1000);
  };
      ...
      <div className="relative z-10 w-full max-w-100">
        <div className="rounded-3xl overflow-hidden shadow-2xl">
          ...
          <div className="bg-gray-50 px-8 py-5">
            ...
            {/* Error */}
            {error && (
              <div className="mb-4 px-4 py-3 rounded-2xl bg-red-50 border border-red-200 text-red-600 text-sm text-center">
                {error}
              </div>
            )}

            {/* Test hint */}
            <div className="mb-4 px-4 py-2.5 rounded-2xl bg-blue-50 border border-blue-100 text-xs text-blue-500 text-center">
              Test: <span className="font-bold">nazmul@example.com</span> — যেকোনো password
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                ...
                className="
                  w-full py-3.5 rounded-2xl bg-white hover:bg-gray-50
