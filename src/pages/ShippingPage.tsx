...
export default function ShippingPage() {
  ...
  // 1. Query API for company-specific footer pages
  const { data, isLoading } = useGetFooterPagesByCompanyQuery(companyKey);

  // 2. Find published shipping page from API response
  const apiPageData = data?.data?.find(
    (p: any) => p.page_type === "shipping" && Number(p.is_published) === 1
  );

  // 3. Fallback priority:
  //    First check state passed via Link (if validated upstream), otherwise use API data
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-2xl border border-gray-100 shadow-sm p-8 sm:p-12 mb-12">
        ...
        {rawContent ? (
          ...
          <p className="text-center text-gray-500 my-8">
            No shipping information available for{" "}
...
//   const pageData = data?.data?.find((p) => p.page_type === "shipping");
